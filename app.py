...
if ab_data_file is not None and countries_file is not None:
    ...
    # ==========================================
    # --- NAVIGATION BUTTONS (SESSION STATE) ---
    # ==========================================
    # Initialize the session state so it defaults to the Probability section
    if 'active_tab' not in st.session_state:
        st.session_state.active_tab = "probability"

    # Create 3 columns for our navigation buttons
    col1, col2, col3 = st.columns(3)
    
    if col1.button("📊 Basic Probability", use_container_width=True):
        st.session_state.active_tab = "probability"
    if col2.button("🧪 A/B Test Analysis", use_container_width=True):
        st.session_state.active_tab = "ab_test"
    if col3.button("🤖 ML Model (Regression)", use_container_width=True):
        st.session_state.active_tab = "ml_model"
    ...
    # ==========================================
    if st.session_state.active_tab == "probability":
        ...
        if obs_diff > 0:
            ...
            direction = "lower"
            display_diff = abs(obs_diff) # Removes the negative sign
        else:
            direction = "the exact same"
            display_diff = 0

        if obs_diff != 0:
            st.info(f"**Observed Difference:**\n\n The treatment group (new page) has a conversion rate that is **{display_diff:.4%} {direction}** than the control group (old page).\n\n Next, navigate to the A/B Testing section to figure out if this tiny difference is statistically significant or just due to random chance.")
        else:
    ...
    # ==========================================
    elif st.session_state.active_tab == "ab_test":
        ...
        st.header("A/B Test Simulations & Z-Test")
        ...
        if p_value < alpha_level:
            ...
            st.success(
                ...
                "We are highly confident that this improvement is real and not just a fluke."
            )
        else:
    ...
    # ==========================================
    elif st.session_state.active_tab == "ml_model":
        # Keep alpha slider available for the ML model section too
        ...
        with st.spinner("Fitting Logistic Regression Model..."):
            try:
                ...
                if significant_vars:
                    ...
                    st.success(
                        f"**What else matters?**\n\n"
                        f"Our deep dive shows that factors like **{', '.join(clean_vars)}** "
